...
export function HabitsList({ date, onCompletedChanged }: HabitsListProps) {
    ...
    //Separar em componente CHeckbox.Root em um componente se não o fizerem
    return (
        <div className='mt-6 flex flex-col gap-3'>
            {habitsInfo?.possibleHabits.map(habit => {
                return (
                    <Checkbox.Root key={habit.id} checked={habitsInfo.completedHabits.includes(habit.id)}
                    onCheckedChange={() => handleToggleHabit(habit.id)} disabled={isDateInPast}
